...
```{r "Angiosperm Tree", echo=FALSE, fig.height=4, fig.width=5}
library(ape)
...
```{r "angiosperms with characters", echo=FALSE, warning=FALSE, fig.height=4, fig.width=5}
...
1. Evolution occuring in each branch of the phylogeny is independent of that occuring in the other branch.
...
```{r "BM model - sigma example", echo=FALSE, fig.height=3, fig.width=5, message=FALSE}
library(phytools)
library(ggplot2)
tree<-pbtree(n=200)
...
```{r "Plot continuous parameter - table.phylo4d", echo=FALSE, message=FALSE}
library(phytools)
library(phylobase)
library(adephylo)
...
```{r "Open seed plant data", warning=FALSE}
require(ape)
...
We will now calculate independent contrasts using the `pic` function in `ape`. First, let's create numeric vectors for four traits: wood density (Wd), shade tolerance (Shade), seed mass (Sm), and nitrogen content (N). A contrast will be calculated for each node. So if there are $n$ species in your tree, $n-1$ contrasts will be estimated. Note that contrasts are estimated for each character individually.

```{r "Calculating Contrasts", warning=FALSE}
...
```{r "Plot contrasts", warning=FALSE, fig.height=9, fig.width=9}
...
```{r "Standard regression", warning=FALSE}
...
You can see that there is not significant effect of wood density on shade tolerance, but that the relation between nitrogen content and seed mass is significant. Now let's look at the same relationships after we corrected for phylogenetic information.

```{r "PIC regression", warning=FALSE}
...
When taking phylogenetic information into account, seed mass is not significantly related to nitrogen contant anymore. This means that the apparent correlation observed on the raw data was an artefact of their evolutionary histories. The other regression, in contrast, now shows a significant relationship between wood density and shade tolerance. This is a positive example of the application of PIC. Indeed, the application of PIC does not always make the relationships less significant. Sometimes, it helps highlight significant relationships that were obscured by the evolutionary history of species.

## Plot contrasts

It is often useful to plot the contrasts to visualize the regression. This allows to check that there indeed a linear relationship between the 2 variables:

```{r "Plot PIC regression", fig.height=3, fig.width=3}
library(ggplot2)
par(mfrow=c(1,2))
...
```{r "Major Axis PIC regression", message=FALSE}
library(smatr)
...
```{r "PGLS ANOVA", fig.height=3, fig.width=4}
# PGLS ANOVA
require(ape)
require(nlme)
...
```{r "logistic regression simulations", message=FALSE}
require(phylolm)
set.seed(123456)
...
```{r "logistic regression - continuous variable"}
...
```{r "logistic regression - categorical variable"}
...
```{r "logistic regression - categorical variable results"}
...
```{r "pPCA", fig.height=4, fig.width=5}
#pPCA
...
Please test a linear model of your choice using Phylogenetic General Least Squares (PGLS) on your data.
