ContinuousML = function (tree, data , directional = FALSE, lambda = 1, kappa = 1, delta = 1, ou = 0, tc = FALSE, mlt = c(), silent=TRUE) {
...
# Basic model info
BTout[[1]] = data.frame(Model = "Continuous", Analysis = "Maximum likelihood", MLattempts = as.numeric(tail(strsplit(FullOut[grep("ML attempt", FullOut)], " ")[[1]], 1)), NumRates = as.numeric(tail(strsplit(FullOut[grep("No of Rates", FullOut)], " ")[[1]], 1)), Lambda = lambda, Kappa = kappa, Delta = delta, OU = ou, Directional = directional, TestCorrel = tc)
...
# Return object of class ContinuousML
class(BTout) = "ContinuousML"
return(BTout)
