RegressionMCMC = function(tree, data, lambda = 1, kappa = 1, delta = 1, ou = 0, it = c(), bi = c(), sa = c(), rd = c(), silent = TRUE) {
...
# Basic model info
BTout[[1]] = data.frame(Model = "Continuous", Analysis = "MCMC", NumRates = as.numeric(tail(strsplit(FullOut[grep("No of Rates", FullOut)], " ")[[1]], 1)), Lambda = lambda, Kappa = kappa, Delta = delta, OU = ou)
...
# # Return object of class RegressionMCMC
class(BTout) = "RegressionMCMC"
return(BTout)
