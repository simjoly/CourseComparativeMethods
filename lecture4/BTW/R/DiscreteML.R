...
DiscreteML = function(tree, data, dependent = FALSE, mlt = c(), res = c(), resall = c(), mrca = c(), fo = c(), et = c(), silent=TRUE) {
...
for (n in 1:nrow(Results)) {
	...
	for (i in 1:4) {
		for (j in 1:4) {
			if (i != j  && sum(i,j) != 5) {
				transmatrix[i,j] = Results[n,grep(tail(paste("q", i, j, sep=""), 1), names(Results))]
			} else transmatrix[i,j] = NaN
		}
	}
	Matrix[[n]] = transmatrix
}
...
# Basic model info
BTout[[1]] = data.frame(Model = "Discrete", Analysis = "Maximum likelihood", MLattempts = as.numeric(tail(strsplit(FullOut[grep("ML attempt", FullOut)], " ")[[1]], 1)), NumRates = as.numeric(tail(strsplit(FullOut[grep("No of Rates", FullOut)], " ")[[1]], 1)), CharSymbols = tail(strsplit(FullOut[grep("Character Symbols", FullOut)], " ")[[1]], 1), Dependent = dependent, Covarion = FALSE)
...
# Return object of class DiscreteML
class(BTout) = "DiscreteML"
return(BTout)	
