...
MultistateMCMC = function (tree, data, res = c(), resall = c(), mrca = c(), fo = c(), et = c(), it = c(), bi = c(), sa = c(), rd = c(), pa = c(), pr = c(), hp = c(), hpall = c(), rj = c(), rjhp = c(), silent = TRUE) {
...
input = c(1,2)	#Multistate, MCMC
input = c(input, paste("lf ./BTout.log.txt"))	#Logfile
if (length(res) > 0) {for (i in 1:length(res)) {input = c(input, paste("res", res[i]))}}
if (length(resall) == 1) {input = c(input, paste("resall", resall))}
...
if (length(pr) > 0) {for (i in 1:length(pr)) {input = c(input, paste("pr", pr[i]))}}
...
# Basic model info
BTout[[1]] = data.frame(Model = "Multistates", Analysis = "MCMC", TreeFile = "tree", DataFile = "data", NumRates = as.numeric(tail(strsplit(FullOut[grep("No of Rates", FullOut)], " ")[[1]], 1)), CharSymbols = tail(strsplit(FullOut[grep("Character Symbols", FullOut)], " ")[[1]], 1), Covarion = FALSE, ReversibleJump = length(which(names(Results)=="Model.string")) > 0)
...
# Return object of class MultistateMCMC
class(BTout) = "MultistateMCMC"
return(BTout)
