...
If you want to use your chronogram for other analyses, you might have to convert it into a `phylo` object. The simplest way I found of doing so is the following:

```{r "Convert_chronogram_to_phylo"}
# Convert chronogram to phylo
rbcl.chrono <- read.tree(text=write.tree(rbcl.chrono))
```
